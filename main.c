...
void EndScreen(hero* h, en* e, int choice);
...
void StoryUntrapped(hero *h, en *e, int choice) {
    ...
    printf("\nIn the center stands an ancient, gnarled tree with a face carved into its trunk.\n");
    ...
    free(Title3txt);
    fclose(Ent);
    printf("\nThe eyes of the face open, revealing a deep, glowing green light.\n");
...
while (fgets(tempBuffer, 1024, buffer)) {
    printf("%s", Title4txt);
}
...
free(Title4txt);
    ...
    if(h->intelligence>20){
        printf("\nYou decipher the runes, gaining valuable information about the path ahead.\n");
...
void Continuation(hero* h, en* e, int choice) {
    ...
    FILE *buffer = fopen("ForestHeart.txt", "r");
    if (buffer == NULL) {
        perror("Failed to open file");
        return;
    }

    char *Title6txt = malloc(1024 * sizeof(char));
    if (Title6txt == NULL) {
        perror("Failed to allocate memory");
        fclose(buffer);
        return;
    ...
    while (fgets(Title6txt, 1024, buffer)) {
        printf("%s", Title6txt);
    }
    ...
    printf("\nThe orb pulses with a warm, inviting light. As you reach out to touch it, the light envelops you, filling you with a sense of peace and power.\n You have found the Heart of the Forest,\n a powerful artifact that can heal the land and bestow great wisdom.\n");
    h->health += 15;
    h->mana += 20;
    h->intelligence += 25;
    h->exp += 40;
...
void Check1(hero* h, en* e, int choice) {
    ...
    if ((check == 1) || (check == 2) || (check == 3)) {
        switch (check) {
            ...
            case 3:
                if (h->agility > 20) {
                    printf("Your superior reflexes are no match for the undead.\n You scale a nearby tree and hide in its leaves.\n As they pass by you, one of them seems to smell you,\n using the opportunity you throw some dirt into their undead eyes and make good your escape!\n");
                    h->exp += 15;
                    Continuation1(h, e, choice);
                } else {
...
void Continuation1(hero* h, en* e, int choice) {
    ...
    printf("\nThe face on the tree is now cruel and malevolent.\n");
    printf("Yet seeing the Forest heart, the ancient lets you pass with a grim face.\n");
    printf("You have a choice, will you help the ancient? Press 'Y' for yes or 'N' for no T_T\n");
...
void Response1(hero* h, en* e, int choice) {
    ...
    if ((Answer == 'Y') || (Answer == 'y')) {
        ...
        if (((h->intelligence + h->mana) > 30) && ((strcmp(h->Name, "Mage") == 0) || (strcmp(h->Class, "Mage") == 0) || (strcmp(h->Name, "Paladin") == 0) || (strcmp(h->Class, "Paladin") == 0))) {
            ...
            h->health += 15;
            h->agility += 15;
            printf("\nYou find a tiny seed in the apple it gave you.\n You decide to hold on to it.\n");

            FILE *EntSeed1 = fopen("Seed_Img.txt", "r");
            if (EntSeed1 == NULL) {
                perror("Failed to open file");
                return;
            }
            ...
            while (fgets(art1, 1024, EntSeed1)) {
                printf("%s", art1);
            }
            fclose(EntSeed1);
            free(art1);
            ...
            fclose(EntSeed);
            printf("\nYou decide to leave the tree alone and head to a nearby village to rest and get ready for your next adventure.\n");
        EndScreen(h,e,choice);
        } else {
            printf("\nMomentarily, the tree glows brightly before gaining its original shade.\n");
            printf("\nHowever it looks saddened by the state of the forest\n");
            printf("\nIt speaks: 'This place is my home, a place with such corruption is unfit for any and unlike most creatures, we Ents can't move about.\n This is the end for me, child. Fret not, for it isn't your fault. I am glad at least someone worthy got the artifact.\n Thank you for making the last moments of this old one bearable.'\n");
            printf("\nThe Ancient smiles as it hands you a golden apple from its branches.\n");
            h->mana += 15;
            h->health += 15;
            h->agility += 15;
            printf("Consuming it has permanently boosted your stats\n");
            printf("\nThe old tree withers away before you.\n You find a tiny seed in the apple it gave you in its last moments.\n You decide to hold on to it.\n");
...
void EndScreen(hero* h, en* e, int choice){
    printf("This is it for now! More adventures soon! Stay Tuned");
    ...
    char* endart=(char)malloc(1024*sizeof(char));
    while (fgets(endart, 1024, End)) {
                printf("%s", endart);
            }
            fclose(End);
            free(endart);
        ...
        printf("Very well, let the games begin\n");
        Story1(h,e, choice);
    }
    else{
        EndScreen(h,e,choice);
